...
def elbo(model, guide, *args, **kwargs):
    ...
    mu = PARAM_STORE['guide_loc']
    logstd = PARAM_STORE['guide_scale_log']
...
def main(args):
    ...
    def guide(data):
        ...
        guide_scale = param("guide_scale_log", torch.tensor(0.)).exp()
        ss = sample("loc", dist.Normal(guide_loc, guide_scale))
        print("loc_sample =", ss.item())
    ...
    assert (param("guide_loc") - 3.0).abs() < 0.1
